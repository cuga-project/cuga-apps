...
> **TL;DR** — If you've built agents before, you already know the model is
> rarely the hard part. The work is in wiring up tools, holding state together
> across a long workflow, adding guardrails, and growing from one agent to
> several without starting over. [CUGA](https://cuga.dev) (`pip install cuga`)
> is a lightweight, open-source agent harness that takes care of that plumbing
> and stays easy to configure. To show what it feels like in practice, we built
> **CUGA Apps**: two dozen working examples that double as copy-and-edit
> templates for real agentic applications. This is the developer tour.
...
harness from IBM Research, hosted at [cuga.dev](https://cuga.dev). We call it a
harness rather than a framework on purpose. It's the runtime that handles the
unglamorous orchestration around a model: planning out a task, calling tools,
writing and running code, keeping intermediate state straight, and retrying
when something fails. You get that machinery for free instead of rebuilding it
every time you start a new agent.

The name captures the idea. You start from a strong generalist and narrow it to
your domain by giving it your tools, your procedure, and your guardrails. The
same harness runs as a single-file app on your laptop and as a self-hosted,
governed deployment in production. It's the engine behind [IBM's Sovereign Core](https://www.ibm.com/docs/en/sovereign-core/1.0.0?topic=services-ai-agent-types) agentic
experience, and it's just as happy powering a weekend project. The rest of this
post is what that looks like from a developer's seat.

## What CUGA gives you out of the box

The fair question to ask of any agent framework is what it saves you from
writing yourself. With CUGA the list is long. The API you actually touch is
small (`CugaAgent(tools=[...])`, then `await agent.invoke(msg)`), but each of
the things below is something you'd otherwise be building and maintaining
yourself. No single one of them is unique to CUGA. What's different is that they
come pre-assembled and benchmarked together, so you configure them instead of
wiring them up.
...
- **Connecting tools.** OpenAPI specs, MCP servers, and plain LangChain
  ...
  REST API, a custom MCP server, and a Python function all bind the same way, so
  you don't write one-off adapters.
- **Keeping a long task coherent.** CUGA plans before it acts and executes with a
  mix of tool calls and generated code (CodeAct). What really matters on long
  horizons is *variable management*: it holds onto intermediate results rather
  than re-deriving (and re-hallucinating) them every turn, and a reflection step
  lets it catch a bad call and re-plan instead of pushing ahead. That's the
  machinery behind landing **#1 on [AppWorld](https://appworld.dev/)** (750 tasks
  across 457 APIs) and **#1 on [WebArena](https://webarena.dev/)** from February
  to September 2025. You don't hand-roll a state machine or a retry loop to hold
  a 20-step task together.
...
- **Going multi-agent.** Any agent can be handed to another agent as a tool. A
  ...
  so growing from one agent to a small team means adding a specialist rather than
...
- **Trading speed for precision.** Fast, Balanced, and Accurate reasoning modes,
  ...
  cloud). You set cost and latency from config, not code.
- **Grounding on your own documents.** A built-in knowledge engine does
  Docling-powered RAG over PDFs, Office files, HTML, and Markdown, at agent or
  session scope, so basic grounding doesn't mean standing up a separate vector
  pipeline.
...
The first word of the name does the work: *Configurable*. CUGA isn't a finished
product you adopt wholesale, and it isn't a black box. It's a strong default you
bend toward your own problem. You point it at your tools, write the procedure
into the system prompt, set whatever policies you care about, and ship. The hard
parts above are already handled, so your job is just the task. The rest of this
post is about what "the task" actually looks like, using apps you can clone
today.

## CUGA Apps: a place to start

You can't really feel "configurable" from a README, so we built **CUGA Apps**
instead: two dozen small, runnable apps. Each is the same short list of parts (a
`CugaAgent`, a tool list, a system prompt) wrapped in a FastAPI server, with a
live panel that shows the agent's state as it works.

They're worth a few minutes just to click through, since they cover tool
composition, stateful tools, procedural planning, and governance on tasks
concrete enough to poke at. Each app leans on one capability in particular, so
together they map out what the framework can actually do:
...
The first two are basic; every agent loops and picks tools. Capabilities 3–6 are
what sort the apps into research, document, and media camps. The last two, code
synthesis and self-correction, are the harder end. Either way, whatever you're
building, some app here already exercises the capability you need, so you start
from working code instead of a blank file.

The tour is useful, but the apps are most valuable later, as **templates**.
They're deliberately small and single-file, so the fastest way in isn't reading
the docs end to end. It's copying the app closest to your idea, swapping its
tools and prompt, and going from there. A few were even generated from a
one-page spec, which tells you how repeatable the shape is.

So let's walk one app end to end to see what "configure and extend" looks like in
practice, then look at the shared plumbing that makes the next app a
...
headlines, an encyclopedia blurb, optional nearby attractions, and an optional
crypto spotlight. It's about 400 lines in a single file, and most of that is
configuration; CUGA does the actual work. Here's how it fits together, and which
lines you'd touch to make it your own.
...
`create_llm` is a small factory: the same app runs on RITS, watsonx, OpenAI,
Anthropic, LiteLLM, or Ollama depending on an env var. Nothing in the app code
knows which model is behind it, and that uniformity comes from CUGA rather than
...
Every inline tool returns that same small envelope: `{"ok": true, "data":
{...}}` when things work, and `{"ok": false, "code": "...", "error": "..."}` when
they don't. It looks trivial, but across all the apps it's the convention that
matters most. It's what lets the agent reason about a failure ("geocoding didn't
work, skip that part") instead of choking on a raw stack trace.

### The system prompt is a procedure

City Beat's prompt doesn't say "be a helpful city assistant." It hands the agent
an ordered playbook: *call `set_current_city`, then `get_session_state` to recall
prior focus topics, then `geocode`; build the briefing from weather + search +
Wikipedia; call `save_briefing` at the end; reply in two short paragraphs.* It
also spells out the anti-hallucination rules ("never make up headlines, weather
numbers, or coordinates; if a tool fails, say so and skip that section"). The
planner follows that procedure, turn after turn, which is where CUGA's structured
planning pays off.
...
state. The moment the agent calls `save_briefing(...)` mid-turn, the panel on the
right redraws the briefing card. The UI isn't a separate copy of the logic; it's
just a view onto state the agent mutated.
...
file you can read in a sitting. City Beat was itself generated from a single
spec ([`cuga_external_app_spec.md`](cuga_external_app_spec.md)), which is about
the best evidence I can give you that the shape is easy to reproduce, by a model
or by you.

The smallest apps go the other way. [**Recipe
...
recipe cards fill in as you chat, with no MCP and no keys beyond your LLM
provider. It's the same shape with fewer parts.

## What two dozen apps taught us

City Beat is one app. Building two dozen of them on the same harness became its
own kind of benchmark, less of accuracy than of ergonomics, and the biggest
...
Most of what else we noticed follows from that:
...
- **The result envelope mattered more than the model.** Apps got reliable once
  ...
  an undeclared one. It's a boring convention that turns out to be load-bearing.
...
- **Hosting capabilities once changed the economics.** Because the shared
  capabilities live in servers the apps point at, most of them need no
...
planning, the self-correction, the tool uniformity, and the governance, which
leaves you thinking about the product: what task to run, what state to track, how
to show it. That's why a new app here takes an afternoon rather than a project.
The sections that follow are really a closer look at the "CUGA handled the rest"
half of that thesis.

## Shared capabilities, hosted once

The capabilities the apps reach for over and over live in **7 MCP servers**,
hosted publicly on IBM Code Engine. There's no auth: point a CUGA app at the URL
and go.
...
That's 36 tools across the seven servers. A small bridge resolves their URLs
automatically, so the same app code talks to the hosted servers from your laptop
or to local copies under Docker, with no edits either way. Because MCP tools are
ultimately just data, there's also a tool explorer for browsing every hosted tool
and calling it from a form before you wire it into an agent. It's a quick way to
sanity-check the plumbing without writing any agent code.

## Packaging know-how as skills

If MCP servers package *tools*, **Agent Skills** package *know-how*: how to do
something well. A skill is a folder in the canonical
...
agent you build afterward can pick it up.

Skills work well with multi-agent setups, too. In the lead-gen app coming up,
each of the seven specialists is its own `SKILL.md`, which is exactly how you add
a new specialist without touching the coordinator.
...
They live in a shared `sqlite-vec` store and are matched semantically, so they
...
The task is ordinary: *find local businesses worth selling to.* Type a place and
...
decision-maker and best-guess email, and a drafted cold email. It's the kind of
thing a human does well but slowly, one business at a time.
...
`delegate_to_<specialist>`. It reasons in delegations rather than raw tool calls,
so its planning surface stays small no matter how many tools sit underneath. Each
specialist is self-contained, with its own toolset, its own prompt, and its own
isolated context and failure surface, so a flaky tool fails a single delegation
...
just composed one level up. The payoff is extensibility: adding a new signal, say
a competitor finder, means dropping in a specialist as its own skill instead of
rewriting the coordinator. The system grows by accretion while the supervisor's
reasoning surface stays as small as it started.

## Two dozen examples to learn from

The apps fall into a few families. A research-and-knowledge cluster pulls
sourced answers out of the open web and the literature: Paper Scout ranks arXiv
and Semantic Scholar papers by citation count, Wiki Dive and Web Researcher do
cited synthesis, and YouTube Research works from transcripts. An
everyday-productivity cluster covers the small, useful things: City Beat's daily
briefing, a multi-day travel planner, a pantry-driven recipe composer, and trail
discovery for a weekend hike. There's an ops corner watching live system metrics
and market prices, and an enterprise example answering questions from real IBM
...
Running one of these is deliberately undramatic: install the app's requirements,
point it at the hosted MCP servers with `CUGA_TARGET=ce` so you don't need any
third-party keys (just an LLM provider), and start it. A couple of commands,
which the repo spells out, and you've got a CUGA agent answering in your browser.
...
app back. City Beat was built exactly that way. More than any feature list,
that's the sign this is a *repeatable pattern* and not a pile of bespoke demos:
it's regular enough for a model to reproduce, which means it's regular enough for
you to learn.

## The takeaway

You get an agent that plans, fixes its own mistakes, collaborates, calls
whatever tools you give it, and stays inside the lines you draw. It's built to be
configured, not just consumed. That makes it a good place to build from, whether
you're knocking out a one-off briefing or standing up a governed production
agent, and you can grow into the deeper features as you need them.

...
`_carbon` foundation. Layout: left input/history/tips column, right results panel.
"""
...
_APP_CSS = """<style>
  ...
  .tips strong { color: var(--cds-text-primary); }
  .tips { font-size: 0.8125rem; color: var(--cds-text-secondary); line-height: 1.6; }
...
<header class="cds-header">
  <div class="cds-header__name"><span class="cds-header__prefix">IBM</span>&nbsp;Paper&nbsp;Scout</div>
  <div class="cds-header__actions">
    <span class="cds-helper-01">arXiv + Semantic Scholar · Powered by CugaAgent</span>
...
<div class="app-intro">
  <div class="app-intro__blurb">
    <strong>Paper Scout.</strong> Searches arXiv and Semantic Scholar for research
    papers, then synthesises findings across sources with citations — or summarises
    a single paper from its arXiv ID.
  </div>
  <div class="app-intro__tools">
    <span class="tools-label">Tools</span>
    <span class="tool-pill">📄 arXiv</span>
    <span class="tool-pill">📚 Semantic Scholar</span>
    <span class="tool-pill">🔗 Paper references</span>
  </div>
</div>
  ...
  <div class="panel">
    <div class="cds-tile">
      ...
      <div class="chat-row">
        ...
        <button class="cds-btn" id="sendBtn" onclick="send()" style="min-width:8rem">Search</button>
      </div>
    </div>
    ...
    <div class="cds-tile tips">
      <div class="card-title">Tips</div>
      <strong>Topic mode</strong> — type a research area and the agent searches
      arXiv + Semantic Scholar, then synthesises findings across papers with
      citations.<br><br>
      <strong>arXiv mode</strong> — paste an arXiv URL or ID (e.g.
      <span class="cds-mono" style="color:var(--cds-link-primary)">2305.11206</span>)
      to get an instant structured summary with key contributions and
      limitations.<br><br>
      <strong>Follow-up</strong> — ask "what does this build on?" to fetch
      reference lists.
    </div>
  </div>
...
async function send() {
  ...
  panel.innerHTML = '<div class="thinking"><div class="cds-spinner"></div> Searching arXiv and Semantic Scholar… this may take 15–30 seconds.</div>'
  ...
  try {
    ...
    panel.innerHTML = '<div class="result-content">' + renderMd(data.answer) + '</div>'
    addHistory(q)
  } catch(err) {

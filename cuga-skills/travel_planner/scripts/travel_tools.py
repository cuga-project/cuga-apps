"""CLI helpers for the travel_planner skill — stdlib only.

Wraps Nominatim, wttr.in, OpenTripMap, Tavily, and Wikipedia REST + action
APIs. The agent invokes this script as a subprocess and parses JSON from
stdout:

    python scripts/travel_tools.py get_wikipedia_article 'Tokyo'
    python scripts/travel_tools.py search_wikipedia 'Tokyo' 5
...
_UA = {"User-Agent": "travel-planner-skill/1.0 (https://skills.sh)"}
...
def search_attractions(
    lat: float, lon: float, category: str = "interesting_places",
    limit: int = 10, radius_m: int = 20000,
) -> dict:
...
_USAGE = """\
usage:
  python scripts/travel_tools.py get_wikipedia_article <title>
  python scripts/travel_tools.py search_wikipedia <query> [max_results=6]
  python scripts/travel_tools.py geocode <place>
  python scripts/travel_tools.py get_weather <city>
  python scripts/travel_tools.py search_attractions <lat> <lon> <category> [limit=10] [radius_m=20000]
  python scripts/travel_tools.py web_search <query> [max_results=5]
